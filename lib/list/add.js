...
function toUiArray (thing, prefix) {
	var result;
	var stack;
	var uiItem;
	...
	function makeHeading (heading, options) {
		...
		uiItem = {
			type: 'view',
			options: Object.assign({}, options, { type: 'Heading', heading: heading }),
			path: prefix,
			children: [],
		};
	...
	if (_.isArray(thing)) {
		...
		thing.forEach(function (item) {
			if (typeof item === 'string') {
				...
				stack[0].children = stack[0].children.concat(toUiArray(item, prefix));
			}
		});
...
function add () {
	...
	var newUiElements = toUiArray(args, '');
