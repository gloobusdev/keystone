...
function field (path, options) {
	...
	if (typeof options.type === 'string') {
		if (!fieldTypes[options.type]) {
			throw new Error('Unrecognised field type: ' + options.type + ' for ' + this.key + ':' + path);
		}
		options.type = fieldTypes[options.type];
	}
	if (typeof options.type !== 'function') {
		throw new Error('Field must be specified with a type function in ' + this.key + ':' + path);
	}
	if (!(options.type.prototype instanceof Field)) {
		...
		if (options.type === String) {
			...
			throw new Error('Unrecognised field constructor: ' + options.type + ' for ' + this.key + ':' + path);
		}
	}
