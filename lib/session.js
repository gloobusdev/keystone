...
exports.keystoneAuth = function (req, res, next) {
	if (!madeREs) {
		...
		if (allowed) {
			...
			whitelistRE = new RegExp('^/' + adminPath + '/api/(' + regexes + ')(/|\\?|$)');
		}
		keystoneRE = new RegExp('^/' + adminPath + '/?$', 'i');
	}
	if (whitelistRE && req.method === 'GET' && whitelistRE.test(req.originalUrl)) {
		return next();
	}
	if (!req.user || !req.user.canAccessKeystone) {
		...
		var from = keystoneRE.test(req.originalUrl) ? '' : '?from=' + req.originalUrl;
