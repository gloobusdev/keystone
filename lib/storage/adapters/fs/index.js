 ...
 * Unzip a package
 */
FSAdapter.prototype.unzipFile = function (file, targetDir, callback) {
	debug('Unzipping file with amd-zip', file);

	var zip = new AdmZip(file.path+path.sep+file.filename);
	...
	try {
		...
		return callback(new Error("Create directory fail: " + targetDir, err));
	}
 ...
 * PDF a file
 */
FSAdapter.prototype.htmlToPdf = function (url, filePath, targetDir, resultFileName, callback) {
	debug('HTML TO PDF with html-pdf: ', url);
		...
		if (fs.existsSync(filePath)) {
			...
			htmlPdf.create(
				htmlContent,
				{
					...
					"zoomFactor": "1",
					"type": "pdf",
					"base":  "file://" + targetDir + "/", //"file:///home/www/your-asset-path",
					/* "phantomArgs":
						[
							"--debug=true",
							"--web-security=false", // take care: enables web security and forbids cross-domain XHR (default is true).
							"--load-images=true",
 ...
 */
				...
				(err, res) => {
					if (err) {
						debug('HTML TO PDF ERROR ', err);
					}
					debug('HTML TO PDF done ', resultFileName, res);
					return callback(resultFileName)
				}
			)
		}
	} catch(err) {
